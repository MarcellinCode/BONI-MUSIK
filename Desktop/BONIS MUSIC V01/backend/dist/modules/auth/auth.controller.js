...
let AuthController = class AuthController {
    ...
    async login(body) {
        ...
        if (!phoneNumber || !password) {
            throw new common_1.BadRequestException('Phone number and password are required');
        }
    ...
    async register(body) {
        ...
        if (!phoneNumber || !password || !fullName) {
            throw new common_1.BadRequestException('Phone number, full name and password are required');
        }
    ...
    async verifyResetCode(body) {
        if (!body.phoneNumber || !body.code) {
            throw new common_1.BadRequestException('Phone number and code are required');
        }
        return this.authService.verifyResetCode(body.phoneNumber, body.code);
    }
    async resetPassword(body) {
        if (!body.phoneNumber || !body.code || !body.newPassword) {
            throw new common_1.BadRequestException('Phone number, code and new password are required');
        }
